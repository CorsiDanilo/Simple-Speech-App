package com.anomalyzed.simplespeechkeyboard.whisper
...
object WhisperCpuConfig {
    val preferredThreadCount: Int
        get() {
            val available = Runtime.getRuntime().availableProcessors().coerceAtLeast(2)
            val highPerf = CpuInfo.getHighPerfCpuCount()
            val preferred = maxOf(if (highPerf > 0) highPerf else available, minOf(available, 4))
            return preferred.coerceIn(2, 4)
        }
}
